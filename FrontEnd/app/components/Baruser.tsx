...
export default function Baruser() {
  ...
  const handleAdditionalInfoSubmit = (additionalInfo: any) => {
    console.log("เพิ่ม Info เรียบร้อย:", additionalInfo);
    setAdditionalInfoConfirmed(true);
    handleShowPayInsurance();
  };
  ...
  return (
    <div className="flex flex-auto md:flex-row justify-center items-center h-full gap-6 sm:gap-6 p-10 mt-24">
      <div className="flex list-none flex-wrap pt-3 pb-4 flex-row justify-center">
        <div className="">
          <button
            className={`text-xs font-bold uppercase px-5 py-3 shadow-lg rounded-tl rounded-bl block leading-normal m-0 ${
              showMemberInfo ? "text-white bg-orange-600" : ""
            }`}
            onClick={handleShowMemberInfo}
          >
            ...
            {showMemberInfo && (
              <div>
                <div className="text-center  relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded mt-4 border border-gray-500">
                  คณะ: เทคโนโลยีสารสนเทศและนวัตกรรมดิจิทัล สาขา:
