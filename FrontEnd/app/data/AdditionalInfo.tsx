...
type AdditionalInfoProps = {
  onSubmitAdditionalInfo: (additionalInfo: any) => void;
};
...
const AdditionalInfo: React.FC<AdditionalInfoProps> = ({
  ...
  const handleChange = (e: any) => {
    ...
    if (name === "studentId") {
      const isValidStudentId = /^[0-9]{0,9}$/.test(value);
      ...
      const isValidTelephoneNumber = /^[0-9]{0,10}$/.test(value);
  ...
  useEffect(() => {
    const { studentId, telephoneNumber, faculty, department, contractDate } =
      additionalInfo;
    const isStudentIdValid = /^[0-9]{9}$/.test(studentId);
    const isTelephoneNumberValid = /^[0-9]{10}$/.test(telephoneNumber);
  ...
  return (
    <div className="flex-auto md:flex-col justify-center items-center h-full gap-6 sm:gap-6 p-10  text-center ">
      <div className="Box w-[409px]  px-[21px] py-[26px] flex-col items-center gap-[13px] inline-flex justify-center">
        ...
        <div className="flex justify-end mt-5 mr-[16px] ">
          <button
            className={`relative rounded-xl px-4 py-2 text-white  flex ${
              isFormComplete ? "bg-[#E4965D]" : " bg-gray-400"
            }`}
        ...
        {!isFormComplete && (
          <div className="text-[#ff4747] mt-2">กรุณากรอกข้อมูลให้ครบถ้วน</div>
        )}
      </div>
    </div>
  );
